...
ERP_CONTRACT = {
    ...
    "zenith": "+Z",
    "nadir": "-Z",
    "erp_x_direction": "U advances through back (-180°), left (-90°), front (0°), right (+90°), back (+180°); seam at ±180°",
...
# Passes whose pixel values are discrete labels must never be interpolated.
...
def world_direction_to_erp(direction: np.ndarray | tuple[float, float, float]) -> tuple[float, float]:
    ...
    Canonical contract (matches the actual SketchUp/Phase 0C camera basis):
      ...
      - bottom = -Z
      - longitude 0 = front
      - longitude +pi/2 = right (-Y), because lon = atan2(-y, x)
      - longitude -pi/2 = left (+Y)
      - longitude pi = back
...
def cubemap_to_erp(cube: Mapping[str, np.ndarray], width: int, interpolation: str = "linear") -> np.ndarray:
    """Stitch a six-face cubemap into an equirectangular panorama using the canonical contract.

    `interpolation` must be "linear" for continuous passes (albedo, AO, depth, normal) and
    "nearest" for discrete label passes (material_id, object_id). Interpolating a label pass
    invents blended IDs that do not exist in the source scene.
    """
    ...
    def remap(mask, face):
        ...
        flag = cv2.INTER_NEAREST if interpolation == "nearest" else cv2.INTER_LINEAR
        ...
        sample = cv2.remap(np.asarray(cube[face]), u, v, flag, borderMode=cv2.BORDER_REPLICATE)
